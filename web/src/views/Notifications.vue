<template>
  <div class="notify-page" v-loading="loading">
    ...
    <div class="channel-grid">
      <el-card class="channel-card">
        <template #header>
          <div class="card-header">
            <div class="channel-name">
              <span class="telegram-logo">T</span>
              <div><b>Telegram</b><small>Bot API</small></div>
            </div>
            <el-tag :type="telegramActive ? 'success' : 'info'">{{
              telegramActive ? $t('common.tagEnabled') : $t('common.tagDisabled')
            }}</el-tag>
          </div>
        </template>
        ...
        <div class="channel-details">
          <div>
            <span>{{ $t('notifications.telegram.botToken') }}</span
            ><b>{{ settings.telegram.botTokenConfigured ? $t('common.keyConfigured') : $t('common.notConfigured') }}</b>
          </div>
          <div>
            <span>Chat ID</span><b>{{ settings.telegram.chatId || '-' }}</b>
          </div>
          <div>
            <span>{{ $t('notifications.telegram.topic') }}</span
            ><b>{{ settings.telegram.messageThreadId || '-' }}</b>
          </div>
        </div>
        <div class="actions">
          <el-button type="primary" @click="openTelegram">{{ $t('notifications.configure') }}</el-button>
          <el-button :disabled="!telegramActive" :loading="testing" @click="testTelegram">{{
      ...
      <el-card>
        <template #header
          ><div class="card-header">
            <span>{{ $t('notifications.subscription.title') }}</span
            ><el-tag type="info" effect="plain">{{ subscriptionSummary }}</el-tag>
          </div></template
        >
    ...
    <el-card>
      <template #header
        ><div class="card-header">
          <span>{{ $t('notifications.events.title') }}</span
          ><el-button text :icon="Refresh" @click="loadEvents">{{ $t('common.refresh') }}</el-button>
        </div></template
      >
      <div v-if="events.length" class="event-list">
        <div v-for="event in events" :key="event.time + event.type + event.message">
          ...
          <div>
            <b>{{ event.message }}</b
            ><small v-if="event.entity">{{ event.entity }}</small>
          </div>
      ...
      <el-form label-position="top" class="notify-form">
        ...
        <el-form-item :label="$t('notifications.telegram.botToken')">
          <el-input
            ...
            :placeholder="settings.telegram.botTokenConfigured ? $t('common.keepEmpty') : '123456789:AA...'"
          />
        </el-form-item>
        ...
        <el-form-item label="Chat ID"
          ><el-input v-model="form.telegram.chatId" placeholder="123456789 / -100... / @channel"
        /></el-form-item>
        <el-form-item :label="$t('notifications.telegram.topic')"
          ><el-input-number v-model="form.telegram.messageThreadId" :min="0" :controls="false"
        /></el-form-item>
...
const subscriptionSummary = computed(() =>
  settings.types.length
    ? t('notifications.subscription.selected', { n: settings.types.length })
    : t('notifications.subscription.allWarnError')
)
function typeLabel(value) {
  const key = String(value).split('.')[0]
  return allTypes.includes(key) ? t(`notifications.eventTypes.${key}`) : value
}
function levelTagType(level) {
  return level === 'error' ? 'danger' : level === 'warn' ? 'warning' : 'info'
}
function applySettings(payload = {}) {
  settings.types = Array.isArray(payload.types) ? payload.types : []
  Object.assign(settings.telegram, payload.telegram || {})
}
...
async function loadEvents() {
  const res = await request.get('/api/notifications/events?limit=30')
...
function openTelegram() {
  ...
  Object.assign(form.telegram, {
    enabled: settings.telegram.enabled,
    botToken: '',
    clearBotToken: false,
    chatId: settings.telegram.chatId || '',
    messageThreadId: settings.telegram.messageThreadId || 0
  })
...
async function save() {
  ...
  try {
    const res = await request.put('/api/notifications/settings', {
      types: form.types,
      telegram: { ...form.telegram, messageThreadId: Number(form.telegram.messageThreadId) || 0 }
    })
    applySettings(res.data)
...
async function testTelegram() {
  testing.value = true
  try {
    await request.post('/api/notifications/test/telegram')
...
async function deleteTelegram() {
  deleting.value = true
  try {
    const res = await request.delete('/api/notifications/channels/telegram')
    applySettings(res.data)
...
const eventsPoller = useIntervalFn(loadEvents, 30000)
onMounted(() => {
  load()
  eventsPoller.start()
})
...
.notify-form :deep(.el-input-number) {
  width: 100%;
}
.notify-form :deep(.el-input-number .el-input__inner) {
  text-align: left;
}
