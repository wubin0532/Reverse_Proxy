...
复查 `aea3f34` 后发现此前测试未覆盖的 8 项问题，本轮已修改实现并加入回归用例：

- TLS 首字节嗅探阻塞串行 Accept：改为有数量上限的并发嗅探，并在关闭监听器时清理待处理连接。
- 歧义路径绕过子规则鉴权：规则匹配前拒绝点路径段、重复斜杠及反斜杠。
- 空体 POST 在 EOF 后重复执行：自动重试仅允许无请求体的 GET / HEAD / OPTIONS。
- 重试重复改写路径、查询参数及代理头：每次尝试从原始入站请求构建。
- 旧请求在热更新后污染缓存：配置与缓存索引同步发布，拒绝变更前的规则快照；未变更规则复用连接池。
- 文件服务符号链接逃逸根目录：使用 `os.OpenRoot` 限定文件访问范围，保持目录内链接可用。
- 备份畸形参数：固定 v1 KDF 参数，提前校验 salt / nonce / 密文长度，并限制导入导出并发。
- 移除前缀丢失编码路径：同步处理解码路径和编码路径，保留剩余路径段含义。

另为四层 TCP / UDP 转发加入共享资源上限，IP / CIDR 名单在规则启动或变更时编译。具体兼容性行为和固定上限见 README 的“代理行为与资源限制”。

本轮验证（本地，未部署）：

- 使用 `go.mod` 指定的 Go 1.26.6 执行 `go vet ./...` 和 `go test -race -count=1 ./...`，全部通过。
- 原通知队列测试在全量验证中出现调度相关失败，已改为用订阅者屏障稳定填满队列；`TestPublishDropWhenQueueFull` 连续运行 30 次通过。
- 前端 2 个测试文件、4 项测试通过；前端源码及嵌入产物未修改。
- Go 1.26.6 下 Linux amd64 / arm64 / armv7 / mips / mipsle 构建通过，产物写入仓库外临时目录。
- `govulncheck` 未发现当前代码调用的已知漏洞；模块层面另有 3 项未被当前代码调用的漏洞提示。

以下保留此前审查的历史记录，其“未发现严重/高危漏洞”等评价仅代表当时覆盖范围，不能替代此次复查结论。

## 实施状态（2026-09-06 更新）

此前实施记录报告其所列问题已修复，验证 `go build` / `go vet` / `go test -race` 全部通过：
...
- 位置：`main.go:157` — `addr := fmt.Sprintf(":%d", *port)`，默认 `:16606` 绑定所有接口。
...
- 利用场景：管理员首次通过 `https://<设备IP>:16606` 登录时，路径上的攻击者可中间人替换证书，窃取管理密码乃至 TOTP 验证码（TOTP 在主动 MITM 下可被实时转发）。
